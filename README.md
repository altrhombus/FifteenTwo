...
- **Muggins (the `mugginsEnabled` house rule) turns the show interactive** — each player counts their own hand/crib and the opponent can peg any missed points. The rule engine is fully unit-tested and the solo-vs-CPU flow is covered by a full-game integration test (the CPU counts perfectly and mugginses your shortfalls). The claim/muggins UI for the online modes (Multipeer, SharePlay, Game Center turn-based) builds and reuses the same engine moves, but — like everything multiplayer above — hasn't been played end-to-end on real hardware. With muggins off (the default) counting is fully automatic, exactly as before.
